...
void MazeGenerator::drawRectMap(std::vector< std::vector < int> >& map, Block &block){
    ...
    for(unsigned int i = 0 ; i < map.size(); ++i){
        for(unsigned int j = 0 ; j < map[0].size(); ++j)
        {
            if(map[i][j] == 0)
            {
                Vector2f blockSize(blocksize,blocksize), blockPosition(blocksize*i+blocksize-(blocksize-20),blocksize*j+blocksize-(blocksize-20));
                block.addBlock(blockSize,blockPosition);
            }
        }
    }
}

bool MazeGenerator::isValid(std::vector< std::vector < int> > & map, int posx, int posy, int dir){
    std::cerr << "lets validate... " << std::endl;

    if((!(posx < 0 )) && (!(posy < 0 )) && (!(posx >= map[0].size()))
        && (!(posy >= map.size())) && ( (map[posy][posx] == 0)) )
    {
            ...
            for(int i = 0; i < 4 ; ++i)
            {
                ...
                if(i != inverseDir[dir])
                {
                    ...
                    if ((posx2 >= 0 ) && (posy2 >= 0 )
                        && (posy2 < map.size()) && (posx2 < map[0].size()))
                    {
                        if ((map[posy2][posx2] == 1))
                            return false;
                    }
                }
...
void MazeGenerator::generateMaze(std::vector< std::vector < int> > & map, int iniX, int iniY){
    ...
    while(randoms.haveNumbers())
    {
        ...
        if(dir != NONE)
        {
            ...
            if(isValid(map,pos.x + dirx[dir], pos.y + diry[dir], dir))
            {
